...
const zoneFlowers = () => {
   ...
   for (let x of flowers) {
        // .includes() has built in functionality to itterate through an array
        if (x.usdaZones.includes(3) && x.usdaZones.includes(8)) {
            filteredFlowers.push(x)
        }
     } 
...
*/
const flowersAsHTML = () => {
    //declare an empty strig
    let flowersHTMLString = ""  // Do not modify
            //use a for loop to itterate the array
    for (const x of flowers) {
                //add article to the front of each object and wrap topic in h1 tags then add h2Colors after it
        flowersHTMLString += `<article> <h1>${x.type}</h1>`
        flowersHTMLString += `<h2>Colors</h2>`
                //add each color wrapped in a sections tag using a nested for loop
    for (const y of x.colors) {
        flowersHTMLString += `<section>${y}</section>`
    }
                //outside the nested for loop add h2USDA Zonesh2
    flowersHTMLString += `<h2>USDA Zones</h2>`
    
                //make ther nested or loop to wrap each zone in a div tag
    for (const z of x.usdaZones) {
        flowersHTMLString += `<div${z}</div>`
    }
                //outside the neted for loop add a cloding article tag
